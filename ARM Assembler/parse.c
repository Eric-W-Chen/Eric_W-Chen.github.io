...
struct parse_node_st * parse_instruction(struct parse_table_st *pt,
    ...
    if ((opcode == OC_DP) && (tp1->id == TK_IDENT) && 
        ...
        np->stmt.inst.type = MEMIMM;
        strncpy(np->stmt.inst.name, tp0->value, SCAN_TOKEN_LEN);
        np->stmt.inst.memimm.rd = parse_reg_to_int(tp1->value);
        np->stmt.inst.memimm.rn = parse_reg_to_int(tp4->value);
        np->stmt.inst.memimm.imm = atoi(tp6->value);
        ...
        np->stmt.inst.type = MEM2;
        strncpy(np->stmt.inst.name, tp0->value, SCAN_TOKEN_LEN);
        np->stmt.inst.mem2.rd = parse_reg_to_int(tp1->value);
        np->stmt.inst.mem2.rn = parse_reg_to_int(tp4->value);
        ...
        scan_table_accept_any_n(st, 6);
            ...
            (tp2->id = TK_COMMA) && (tp3->id == TK_IDENT) && 
            (tp4->id == TK_COMMA) && (tp5->id == TK_IDENT)) {
        /* Parse mul3 instructions: mul reg, reg, reg */

        np->stmt.inst.type = MUL3;
        strncpy(np->stmt.inst.name, tp0->value, SCAN_TOKEN_LEN);
        np->stmt.inst.mul3.rd = parse_reg_to_int(tp1->value);
        np->stmt.inst.mul3.rn = parse_reg_to_int(tp3->value);
        np->stmt.inst.mul3.rm = parse_reg_to_int(tp5->value);
        ...
        scan_table_accept_any_n(st, 6);
            ...
            (tp2->id = TK_COMMA) && (tp3->id == TK_IDENT)) {
        /* Parse mov instructions: mov reg, reg  */

        np->stmt.inst.type = DP3;
        strncpy(np->stmt.inst.name, tp0->value, SCAN_TOKEN_LEN);
        np->stmt.inst.dp3.rd = parse_reg_to_int(tp1->value);
        np->stmt.inst.dp3.rm = parse_reg_to_int(tp3->value);
        ...
        /* Parse mov instructions: mov reg, imm */

        np->stmt.inst.type = DPIMM;
        strncpy(np->stmt.inst.name, tp0->value, SCAN_TOKEN_LEN);
        np->stmt.inst.dpimm.rd = parse_reg_to_int(tp1->value);
        np->stmt.inst.dpimm.imm = atoi(tp3->value);
        ...
        scan_table_accept_any_n(st, 4);
            ...
            (tp2->id = TK_COMMA) && (tp3->id == TK_IDENT)) {
        /* Parse mov instructions: cmp reg, reg */

        np->stmt.inst.type = CMP;
        strncpy(np->stmt.inst.name, tp0->value, SCAN_TOKEN_LEN);
        np->stmt.inst.cmp.rd = parse_reg_to_int(tp1->value);
        ...
        scan_table_accept_any_n(st, 4);
            ...
            (tp2->id = TK_COMMA) && (tp3->id == TK_IMM)) {
        /* Parse mov instructions: cmp reg, imm */

        np->stmt.inst.type = CMPIMM;
        strncpy(np->stmt.inst.name, tp0->value, SCAN_TOKEN_LEN);
        np->stmt.inst.cmpimm.rd = parse_reg_to_int(tp1->value);
        np->stmt.inst.cmpimm.imm = atoi(tp3->value);
...
void parse_tree_print_inst(struct parse_node_st *np, int level) {
    ...
    } else if (np->stmt.inst.type == MUL3) {
        printf("mul3 (%s) %d %d %d\n", np->stmt.inst.name,
                np->stmt.inst.mul3.rd, np->stmt.inst.mul3.rn, np->stmt.inst.mul3.rm);
