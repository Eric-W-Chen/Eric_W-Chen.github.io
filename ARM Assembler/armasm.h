...
enum parse_opcode_enum {OC_DP, OC_CMP, OC_MEM, OC_MUL, OC_LS, OC_B, OC_BX, OC_NONE};
...
enum parse_inst_enum {DP3, DPIMM, CMP, CMPIMM, MEM2, MEM3, MEMIMM, MUL3, LS,B, BX};

struct parse_node_st {
    enum parse_stmt_enum type;
    union {
        struct {char label[SCAN_TOKEN_LEN];} dir;
        struct {
            ...
            union {
                ...
                struct {int rd; int rn; int imm;} dpimm;
                struct {int rd; int rn; int rm;} cmp; 
                struct {int rd; int rn; int imm;} cmpimm;
                struct {int rd; int rn; int rs; int rm;} mul3;
                struct {int rd; int rn; int rm;} mem2;
                struct {int rd; int rn; int rm;} mem3;
                struct {int rd; int rn; int imm;} memimm;
                struct b {char label[SCAN_TOKEN_LEN];} b;
                struct {int rd; int rn; int rm; int shift;} ls;
                struct bx {int rn;} bx;
            };
        } inst;
        struct {
